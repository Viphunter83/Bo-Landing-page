...
- [x] Создана обертка `getTenantCollection('...')` в `app/lib/db/tenant_db.ts`.
- [x] Заменены все вызовы `collection(db, '...')` на `getTenantCollection('...')` в `app/admin` и `app/lib/logger.ts`.
- [x] Обновлены Firebase Security Rules для поддержки структуры `tenants/{tenantId}/...` и изоляции данных.
- [x] Настроен Vercel для деплоя нового тенанта `luna_hcmc` с соответствующими переменными окружения.
- [ ] Добавить поле `tenantId` в профиль пользователя в Firebase Auth (через Custom Claims) для более быстрой проверки в правилах.
- [ ] Завершить миграцию всех исторических данных в структуру `tenants/bo_dubai/...`.

---
Этот подход превратил проект в полноценную SaaS-платформу.
